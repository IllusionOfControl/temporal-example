...
class OrderProcessingWorkflow:
    ...
    async def run(self, request: dict) -> str:
        ...
        try:
            ...
            workflow.logger.error(f"Ошибка в процессе заказа. Запуск Saga компенсаций. Причина: {e}")
            self.status = "Отмена заказа (Saga Rollback)"
            ...
            for compensation in reversed(compensations):
                try:
                    ...
                    # Логируем, но не прерываем откат остальных шагов
                    workflow.logger.error(f"Ошибка при выполнении компенсации: {comp_err}")

            # Пробрасываем ошибку дальше, чтобы Workflow завершился со статусом Failed
            raise e
